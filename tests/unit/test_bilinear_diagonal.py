...
def test_bilinear_diagonal_multiplication_shape():
    ...
    y = sparseops_backend.bilinear_diagonal_matvec_mt(Q, bias, num_threads)
...
def test_bilinear_diagonal_multiplication__with_transformed_input_correctness_parametrized(M, K, sparsity):
    sparse_matrix = generate_sparse_matrix(M, K, sparsity)
...
def test_bilinear_diagonal_multiplication__with_standard_input_correctness_parametrized(M, K, sparsity):
    sparse_matrix = generate_sparse_matrix(M, K, sparsity)
    quasi_dense = sparseops_backend.convert_to_quasi_dense(sparse_matrix)
    input_vector = np.random.rand(K).astype(np.float32)
    Q = quasi_dense
    ...
    # Test the bilinear diagonal multiplication
    y = sparseops_backend.bilinear_diagonal_matvec_mt(Q, input_vector, bias, num_threads)
...
def test_bilinear_diagonal_multiplication_threads(threads):
    ...
    y = sparseops_backend.bilinear_diagonal_matvec_mt(Q, bias, threads)
