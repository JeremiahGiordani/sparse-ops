...
SparseOnnxModel::SparseOnnxModel(const std::string &onnx_path) {
    ...
    for (size_t idx = 0; idx < graph.node_size(); ++idx) {
        ...
        if (op == "Conv") {
            ...
            cattr.sentinel_off = size_t(cattr.Cin) * cattr.H_in * cattr.W_in; // one-past
            ...
            for (uint32_t pw = 0; pw < cattr.W_out; ++pw) {
                ...
                size_t offC;
                if ((unsigned)ih < (unsigned)cattr.H_in && (unsigned)iw < (unsigned)cattr.W_in) {
                    offC = ( size_t(km.cin) * cattr.H_in + size_t(ih) ) * cattr.W_in + size_t(iw);
                } else {
                    offC = cattr.sentinel_off;
                }
                cattr.patch_indices[pbase + k] = offC;
                }
            }
            }
